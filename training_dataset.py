...
class MyDataset(Dataset):
    def __init__(self, split = 0, training = False, validation = False, desired_shape = (256, 256, 256)):
        ...
        if training or validation:
            self.files = np.load("splits/train_{}.npy".format(split))
        else:
            self.files = np.load("splits/test_{}.npy".format(split))

        fold = 0
        
        if validation:
            self.files = np.array_split(self.files, 10)
            self.files = self.files[fold]
        elif training:
            self.files = np.array_split(self.files, 10)
            self.files = np.concatenate(np.delete(self.files, fold, 0))
    ...
    def load_complete_dataset(self):
        for index in range(len(self.files)):
            ...
            data_path = "hgn_data/volume_{}.nrrd".format(self.files[index])
            
            image = sitk.ReadImage(data_path)
            img = sitk.GetArrayFromImage(image).transpose(2, 1, 0)

            coords_path = "hgn_data/srep_{}_points.npy".format(self.files[index])
