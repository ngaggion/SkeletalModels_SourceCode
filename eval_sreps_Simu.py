...
preds = os.listdir('Results/Simulated/Simu')

df = []

for pred in preds:
    srep = np.load('Results/Simulated/Simu/'+pred)
    med = medialness(srep)

    gt = np.load('Results/Simulated/GT/'+pred)
    angle = angles_between_sreps(srep,gt)

    id = pred.split('_')[1].split('.')[0]
    surface_name = "hgn_data/surface_"+id+".vtp"
    ...
    if len(df) == 0:
        df = pd.DataFrame([[med,angle,ort]],columns=['Medialness','Angle', 'Orthogonality'])
    else:
        df = pd.concat([df,pd.DataFrame([[med,angle,ort]],columns=['Medialness','Angle', 'Orthogonality'])])

df.to_csv('Results/Simulated/Results_Sreps.csv')