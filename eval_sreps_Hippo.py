...
models = [m for m in models if 'GT' not in m]
...
for model in models:
    ...
    for pred in preds:
        ...
        if len(df) == 0:
            df = pd.DataFrame([[model, med,angle, ort]],columns=['Model', 'Medialness','Angle', 'Orthogonality'])
        else:
            df = pd.concat([df,pd.DataFrame([[model, med, angle, ort]],columns=['Model', 'Medialness','Angle', 'Orthogonality'])])

df.to_csv('Results/Finetune/Results_Sreps.csv')